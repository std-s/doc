...
（1）初始化成功，只能调用 login，不能调用其他接口；<br></br>
...
| 参数名称 | 参数类型                                             | 是否必填 | 描述       |
| -------- | ---------------------------------------------------- | -------- | ---------- |
| config   | [OIMInitConfig](docs/sdks/Class/User/InitConfig.mdx) | 是       | 初始化参数 |
...
| platform       | [Platform](/sdks/enum/web&uniapp)      | 是       | 平台号，用于区分当前登录平台                                                 |
...
| object_storage | [ObjectStorage](/sdks/enum/web&uniapp) | 是       | IM 文件资源存放方式                                                          |
| log_level      | [LogLevel](/sdks/enum/web&uniapp)      | 是       | SDK 日志打印级别                                                             |
